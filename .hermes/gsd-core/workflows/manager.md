...
- **On Hermes Agent:** a backgrounded agent cannot nest the pipeline's subagents, so run the chosen plan/execute step(s) **inline** via their handlers below (in order), then run the inline discuss. There is no overlap.
...
Planning runs autonomously. **First resolve the runtime.** On Hermes Agent a backgrounded agent has no `Agent`/`Task` tool, so it cannot spawn the plan-checker the pipeline relies on — backgrounding it there silently turns `workflow.plan_check` into a self-check. So run plan **inline** on Hermes Agent, and **background** it only on runtimes where a backgrounded agent can still nest subagents.

```bash
RUNTIME=$(gsd_run query config-get runtime --default claude 2>/dev/null || echo "claude")
```

**If `RUNTIME` is `claude` (Hermes Agent):** Run plan inline so the plan-checker and quality gates actually run — do NOT wrap it in `Agent(run_in_background=true, …)`:
...
Execution runs autonomously. **First resolve the runtime.** On Hermes Agent a backgrounded agent has no `Agent`/`Task` tool, so it cannot spawn the per-plan worktree-isolated executors or the verifier — backgrounding it there silently disables `workflow.use_worktrees` isolation and `workflow.verifier`. So run execute **inline** on Hermes Agent, and **background** it only on runtimes where a backgrounded agent can still nest subagents.

```bash
RUNTIME=$(gsd_run query config-get runtime --default claude 2>/dev/null || echo "claude")
```

**If `RUNTIME` is `claude` (Hermes Agent):** Run execute inline so worktree isolation and the verifier actually run — do NOT wrap it in `Agent(run_in_background=true, …)`:

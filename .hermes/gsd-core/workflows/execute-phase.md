...
- **Other runtimes:** If `Agent`/`agent` tool is genuinely unavailable (e.g. a backgrounded
  Hermes Agent agent per #853, or a non-Claude runtime), use sequential inline execution as
...
@C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/agent-contracts.md
@C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/context-budget.md
@C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/gates.md
</required_reading>
...
# Only applies to Hermes Agent (isolation="worktree" is Claude-Code-specific).
...
`USE_WORKTREES` is also automatically set to `false` for the duration of a run when `worktree base-check` detects that the orchestrator HEAD has diverged from the worktree fork base (the #683 condition — e.g. an unmerged milestone or feature branch). This check runs only when `RUNTIME=claude` because `isolation="worktree"` is a Hermes Agent-specific feature; other runtimes do not use it. The auto-degrade prints a one-line warning to stderr and falls through to the sequential path so executors do not hit the exit-42 worktree-branch-check halt. To restore parallel worktree execution, set `worktree.baseRef:"head"` in `.hermes/settings.local.json` (or run `gsd-tools worktree set-baseref`) — this makes the fork base track the live HEAD instead of a fixed remote ref. The `worktree-branch-check` exit-42 guard inside each executor remains in place as a backstop.
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/planner-antipatterns.md
...
```
Pure doc-only / config-only / test-only tasks return `is_behavior_adding=false` and are exempt. When the gate trips, Read `C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/execute-mvp-tdd.md` for the exact halt report format.
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/workflows/execute-plan.md` **inline**
...
per-agent branch namespace `worktree-agent-*`, NOT the `.hermes/worktrees/` path: the
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       <execution_context>
       @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/workflows/execute-plan.md
       @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/templates/summary.md
       @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/checkpoints.md
       @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/tdd.md
       @C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/references/executor-examples.md'}
       </execution_context>
...
Read and follow `C:/Users/hufen/projects/quick-translate/.hermes/gsd-core/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
